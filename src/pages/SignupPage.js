...
import { postSignup } from '../components/services/linkr.js';
import { Container,BlackContainer,GrayContainer, Title,
        Description, Form, Input, Entrar, ErrorMessage, GoToSingUp} from './SigninPage.js'
            ...
            <GrayContainer>
                <Form onSubmit={loginInfo}>
                    ...
                    <Input type="password" placeholder=' password' onChange={event => setSignup({ ...signup, password: event.target.value })}
                        disabled={disableForm} required />
                    <Input type="text" placeholder=' username' onChange={event => setSignup({ ...signup, username: event.target.value })}
                        disabled={disableForm} required />
                    <Input type="text" placeholder=' picture url' onChange={event => setSignup({ ...signup, pictureUrl: event.target.value })}
                        disabled={disableForm} required />
                    ...
                    <Entrar disabled={disableForm} cor={corEntrar} onClick={userLogin} type="submit">
                        {disableForm ? <div><Grid color='white' radius="10" heigth="90" width="90" /></div> : "Sign Up"}
                    </Entrar>
                </Form>
                <GoToSingUp onClick={()=>{navigate('/')}}>Switch back to log in</GoToSingUp>
            </GrayContainer>
        </Container>
    )
}
